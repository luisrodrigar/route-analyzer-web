	...
	renderBarLap(lap, xScale, yScale){
 		return lap.tracks.map((track,index) =>
                <rect className={'lap ' + lap.index + ", trackpoint " + index }
                      x={xScale(track[0])} y={yScale(track[1])} fill={lap.color}
                      height={this.props.height - this.props.padding - yScale(track[1])}
                      width={0.5} key={lap.index + "_" + index}/>)
	};
	...
	render(){
		...
		return(
          ...
          { this.props.laps.map(lap =>
              this.renderBarLap(lap, this.props.xScale, this.props.yScale))}
            <InformationTooltip ref="tooltip" xScale={this.props.xScale}
                                ...
                                handleMouseOver={this.props.handleMouseOver} laps={this.props.laps}
                                data={this.props.data} legend={this.props.legend}
                                trackpoint={{ date, data }}
            />
